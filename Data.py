...
class DBView():    
    def __init__(self, currentWindow):
        ...
        TF = self.topFrame
        TF.grid(row = 0, column = 0, padx = 10, pady = (10, 0), sticky = "new", columnspan = 2)
        # left side bar to toggle on/off player/match views
        self.leftToggleBar = ctk.CTkScrollableFrame(self)
        LB = self.leftToggleBar
        LB.grid(row = 1, column = 0, padx = (5, 0), pady = (0, 0), sticky = "nsw")
        # where the data is displayed
        self.mainFrame = ctk.CTkFrame(self)
        MF = self.mainFrame
        MF.grid(row = 1, column = 1, padx = 5, pady = 10, sticky = "nsew")
